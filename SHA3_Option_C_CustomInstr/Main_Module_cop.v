...
module main(
    ...
    // extended: [17]=is_cop, [16:14]=cop_func(=funct3), [13:0]=core
    wire [17:0] id_control_sig_temp = { id_is_cop, id_funct3, id_control_core };

    wire [17:0] id_control_sig = cntrl ? id_control_sig_temp : 18'b0;
    wire [17:0] ex_control_sig;
    ...
    // ----- SHA-3 coprocessor (Option C) -----
    wire        ex_is_cop   = ex_control_sig[17];
    wire [2:0]  ex_cop_func = ex_control_sig[16:14];
    wire [31:0] cop_result;

    sha3_cop U_cop (
        .clk(clk),
        .rst(rst),
        .cop_en(ex_is_cop),
        .cop_func(ex_cop_func),
        .rs1_val(alu_in1),     // forwarded rs1
        .rs2_val(alu_in21),    // forwarded rs2
        .cop_result(cop_result)
    );

    // For POLL/READ, the result rides the ALU path so normal forwarding and
    // writeback apply. POLL=011, READ=100.
    wire ex_cop_read = ex_is_cop & (ex_cop_func==3'b011 | ex_cop_func==3'b100);
    wire [31:0] ex_result = ex_cop_read ? cop_result : ALU_Result;

    wire [32:0] ex_alu = {ex_zero, ex_result};
